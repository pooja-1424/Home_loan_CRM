    ...
    <div class="row">
        <div class="col-xxl">
            <div class="card mb-4">                   
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="mb-0">Payout Details</h5>
                        <small class="text-muted float-end">                             
                            @can('sanction-create')
                            <a href="{{ route('payouts.create') }}" class="btn btn-primary" role="button">Add Payout</a>
                            @endcan
                        </small>
                 </div>
                ...
                <div class="card-body">
                    <h5 class="card-header">Bank Payout List</h5><br>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr class="text-nowrap">
                                    ...
                                    <th>Incentive Releasestate</th>
                                    <th>Condition</th>
                                    <th>Cutout Statement</th>
                                    <th>Extra Payout</th>
                                    <th>Remark</th>
                                </tr>
                            </thead>
                            <tbody class="table-border-bottom-0">
                                ...
                                @if (isset($bankdata))
                                @forelse ($bankdata as $bankpayout)
                               
                                <tr>
                                    {{-- <td>                                                                 
                                        @can('sanction-delete')
                                        <a href="javascript:void(0)" id="status{{$data->sanction_id}}" title='@php echo ($Sanction->sanction_status==0) ? "Inactive" : "Active"; @endphp' onclick="status('{{ $Sanction->sanction_id }}','{{ $Sanction->sanction_status }}','sanctionstatus')">
                                    ...
                                    <td>{{ ++$i }}</td>
                                    <td>{{ $bankpayout->bank_name }}</td>
                                    <td>{{ $bankpayout->start_date }}</td>
                                    <td>{{ $bankpayout->end_date }}</td>
                                    <td>{{ $bankpayout->min_loan }}</td>
                                    <td>{{ $bankpayout->max_loan }}</td>
                                    <td>{{ $bankpayout->loan_type }}</td>
                                    <td>{{ $bankpayout->frequency }}</td>
                                    <td>{{ number_format($bankpayout->rate_of_commission * 100, 2) }}%</td>
                                    <td>{{ $bankpayout->incentive_releasestte }}</td>
                                    <td>{{ $bankpayout->condition }}</td>
                                    <td>{{ $bankpayout->cutout_statement }}</td>
                                    <td>{{ $bankpayout->extra_payout }}</td>
                                    <td>{{ $bankpayout->remark }}</td>
                                    <td>
                                        <a class="btn btn-showid1" href="{{ route('payouts.show',$bankpayout->payout_id ) }}"><i class="fa fa-eye"></i></a>
                                    </td>
                                    <td>
                                    @can('sanction-edit')
                                        <a class="btn btn-editid1" href="{{route('payouts.edit',$bankpayout->payout_id)}}"><i class="fa fa-pencil"></i></a>
                                    @endcan
                                    </td>                                    
                                </tr>                                    
                                @empty
