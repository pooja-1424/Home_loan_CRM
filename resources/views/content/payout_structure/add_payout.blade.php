...
@endsection
@section('content') 
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Horizon-</span> Add Payout</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('payouts.store') }}" method="POST">
                                ...
                                <div class="input-group input-group-merge">                     
                                    <select class="form-control" name="bank_name[]" multiple data-placeholder="Select an Option">
                                        @foreach(App\Models\Bankname::all() as $cList => $label)
                                            <option value="{{ $label->bank_name }}" {{ in_array($label->bank_name, old('bank_name', [])) ? 'selected' : '' }}>
                                                {{ $label->bank_name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                                <div class="input-group input-group-merge">                                    
                                    <input type="number" name="min_loan" id="min_loan" value="{{ old('min_loan') }}" class="form-control"  placeholder=" Minimum Loan Amount"/>
                                </div>
                                @error('min_loan')
                                    <div style="color: red">{{ $message }}</div>
                                @enderror                                
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                                <div class="input-group input-group-merge">                                    
                                    <input type="number" name="max_loan" id="max_loan" value="{{ old('max_loan') }}" class="form-control"  placeholder="Maximum Loan Amount"/>
                                </div>
                                @error('max_loan')
                                    <div style="color: red">{{ $message }}</div>
                                @enderror                                
                            ...
                            <div class="col-sm-4">
                                <div class="input-group input-group-merge">
                                    <select class="form-control" name="frequency" value="{{ old('frequency') }}">                                                                                                                                                           
                                        <option value="{{ old('frequency') }}">Select</option>                                                                             
                                        <option  value="Monthly" {{ old('frequency') == "Monthly" ? 'selected' : '' }}>Monthly</option>
                                        <option  value="Yearly" {{ old('frequency') == "Yearly" ? 'selected' : '' }}>Yearly</option>                                         
                                    </select>     
                                    {{-- <input type="text" name="frequency" id="frequency" value="{{ old('frequency') }}" class="form-control"  placeholder="Frequency" />                                        --}}
                                </div>
                                @error('frequency')
                                    <div style="color: red">{{ $message }}</div>
                                @enderror                               
                        ...
                        <label class="col-sm-2 form-label" for="Date">Rate Of Commission</label>
                            <div class="col-sm-4">
                                <div class="input-group input-group-merge">
                                    <input type="numeric" name="rate_of_commission" id="rate_of_commission" value="{{ old('rate_of_commission') }}" class="form-control"  placeholder="Rate Of Commission" />
                                </div>
                        ...
                        <label class="col-sm-2 form-label" for="Amount">Incentive Releasestate</label>
                            <div class="col-sm-4">
                                <div class="input-group input-group-merge">                                    
                                    <select class="form-control" name="incentive_releasestte" value="{{ old('incentive_releasestte') }}">                                                                                                                                                           
                                        <option value="{{ old('incentive_releasestte') }}">Select</option>                                                                             
                                        <option  value="First Disbursement" {{ old('incentive_releasestte') == "First Disbursement" ? 'selected' : '' }}>First Disbursement</option>
                                    </select> 
                                    {{-- <input type="text" name="incentive_releasestte" id="incentive_releasestte" value="{{ old('incentive_releasestte') }}" class="form-control"  placeholder="Incentive Releasestate"> --}}
                                </div>
                            ...
                            <div class="row alignb" style="border:1px solid rgb(186, 183, 183);padding-top:25px;padding-bottom:25px;border-radius:10px;"> 
                                <div class="col-sm-3">
                                    ...
                                    <select class="form-control dynamic" id="type-select" name="condition[]" multiple onchange="Condition(Array.from(this.selectedOptions, option => option.value));">
                                        ...
                                        <option value="cutout_statement" {{ in_array('cutout_statement', old('condition', [])) ? 'selected' : '' }}>Cutout Statement</option>
                                        <option value="extra_payout" {{ in_array('extra_payout', old('condition', [])) ? 'selected' : '' }}>Extra Payout</option>
                                        <option value="remark" {{ in_array('remark', old('condition', [])) ? 'selected' : '' }}>Remark</option>
                                      </select>
                                      @error('condition')
                                      <div style="color: red">{{ $message }}</div>
                                  @enderror     
                            ...
                            <br><br>
                        <div class="row justify-content-end" style="margin-left: 30%;">                           
                            <div class="col-sm-10">
